...
import { playlist as playlistData } from "../data/playlist.js";
...
const togglePlay = () => {
  ...
  if (isPlaying.value) {
    ...
    audio
      ...
      .catch((err) =>
        console.log("Interação prévia do usuário necessária:", err),
      );
  }
};
...
<template>
  <div
    class="flex flex-col h-full text-black select-none font-mono text-sm bg-[#c0c0c0]"
  >
    <div
      class="bg-[#0f172a] p-3 m-2 rounded shadow-[inset_2px_2px_0px_rgba(0,0,0,1)] border border-white/10 text-[#00ff00] flex flex-col gap-2"
    >
      <div
        class="flex justify-between items-center bg-black/40 p-2 rounded border border-green-900/30"
      >
        <div class="truncate w-2/3">
          <span
            class="text-xs text-green-700 block uppercase tracking-wider text-[10px]"
            >Música Atual</span
          >
        ...
        <div class="text-right">
          ...
          >
          <div
            class="text-xl font-bold font-mono tracking-widest text-orange-400"
          >
    ...
    <div class="grid grid-cols-4 gap-2 px-2 pb-2">
      <button
        ...
        :class="{
          'bg-zinc-400 shadow-[inset_1px_1px_0px_rgba(0,0,0,1)]': isPlaying,
        }"
      >
    ...
    <div
      class="flex-1 p-2 m-2 mt-0 bg-white border border-gray-400 shadow-[inset_1px_1px_0px_rgba(0,0,0,1)] overflow-y-auto max-h-[140px]"
    >
      <div
        class="text-[11px] uppercase tracking-wider text-gray-500 font-bold mb-1 border-b pb-1 border-dotted border-gray-300"
      >
        Winamp Playlist
      </div>
      <ul>
